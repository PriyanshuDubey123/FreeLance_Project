...
const PYQVideoSolutions = () => {
  ...
  return (
    <div className=" mx-auto p-4 w-[100vw] absolute top-10 ">
      <div className="flex justify-center mb-4">
        <button
          className={`px-4 py-2 ${activeTab === 'Engineering' ? 'border-b-2 border-blue-500' : ''}`}
          onClick={() => setActiveTab('Engineering')}
        >
          Engineering
        </button>
        <button
          className={`px-4 py-2 ml-4 ${activeTab === 'Medical' ? 'border-b-2 border-blue-500' : ''}`}
      ...
      {activeTab === 'Engineering' && (
        <div>
          <div className="mb-4 p-4 border rounded">
            <h2 className="text-xl font-bold">JEE Main (Imp. PYQs from 2023-2019)</h2>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white p-4 border-t">
            ...
            <p className="text-center mt-2 text-sm">Single purchase will unlock all</p>
          </div>
        </div>
      )}
      {activeTab === 'Medical' && (
